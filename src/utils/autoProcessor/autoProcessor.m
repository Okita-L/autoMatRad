classdef autoProcessor < handle
    ...
    methods
        ...
        function [DIJMatPath,resultGUIPath] = autoGenerateDIJandResultGUI(obj, path_of_CSTMat, path_of_STFMat, path_of_DIJMat, path_of_resultGUI)
            ...
            for i=1:numel(STFMatFiles)
                ...
                if exist('resultGUI','var'); clear resultGUI; end
                ...
                %% 计算DIJ
                dijNameWithEXT = [fileName,'_dij.mat'];
                dijSavepath = fullfile(path_of_DIJMat, dijNameWithEXT);
                calcDIJ = true;
                if exist(dijSavepath, 'file')
                    ...
                    try
                        ...
                        fprintf("Ops!----加载已存在的 DIJ 文件 %s 出错\n" + ...
                            "         尝试重新计算中\n", dijNameWithEXT);
                        try 
                            delete(dijSavepath);
                            fprintf('    Info!----已删除损坏的旧文件。\n');
                        catch
                            fprintf('    Warning!----无法删除旧文件，请手动检查权限。\n');
                            continue;
                        end
                    end
                end
                ...
                try 
                    ...
                    fprintf(['Damn!----病人 %s 的 resultGUI 计算失败\n' ...
                        '错误信息: %s\n'], fileName, ME.message);
                    if exist('resultGUI','var'); clear resultGUI newCst; end;
                    continue;
                end
            ...
            %% 返回实际存放路径
            DIJMatPath = path_of_DIJMat;
            resultGUIPath = path_of_resultGUI;
        end
        ...
        function autoGetOrgCSV(obj, path_of_CSTMat, path_of_STFMat, path_of_DIJMat,path_of_resultGUI, path_of_OrgCSV)
            ...
            calcDIJandResGUI = false;
            if isempty(path_of_DIJMat)
                if ~isempty(obj.path_of_DIJMat)
                    path_of_DIJMat = obj.path_of_DIJMat;
                else
                    fprintf("Info!----导入失败: 无法找到原始 DIJMat 文件位置\n" + ...
                        "         因为传入路径为空\n");
                    calcDIJandResGUI = true;
                end
            end
            if isempty(path_of_resultGUI)
                if ~isempty(obj.path_of_resultGUI)
            ...
            for i=1:numel(dijMatFiles)
                ...
                curDIJMat = dijMatFiles(i);
                fprintf('Wait!----正在处理第 %d 个病人数据 %s \n',i,curDIJMat.name);
                %% 导入数据
                try
                    % 均指文件名 不含扩展
                    [~,dijName,~] = fileparts(curDIJMat.name);
                    fileName = dijName(1:end-4);
                    ...
                    % load dij resultGUI
                    if ~exist(fullfile(path_of_DIJMat,[dijName,'.mat']))
                        fprintf('Ops! ----找不到匹配的 DIJ ：%s 跳过 \n', dijName);
                        continue
                    end
    ...
    methods
        ...
        function getALLOrgCSV(obj, cst, ct, stf, pln, dij, resultGUI, fileName, path_of_OrgCSV)
            if nargin == 7
                path_of_OrgCSV = obj.path_of_OrgCSV;
            end
            % 
